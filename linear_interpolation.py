...
# ##### Linear Interpolate Using Native Numpy #####
...
#     interior_mask = (np.where(mask)[0] > first) & (np.where(mask)[0] < last)
#     if np.all(interior_mask):
#         arr[mask] = np.interp(np.where(mask)[0], x, y)

#     return arr

# # Usage
# data = np.array([1, 2, np.nan, np.nan, 7, 8])
# interpolated_data = linear_interpolate(data)
# print(interpolated_data)


# ##### Linear Interpolation Using Pandas Series

# Sample data with missing values (None or np.nan)
# data = np.array([1, 2, np.nan, np.nan, 5, 6, np.nan, 8])
